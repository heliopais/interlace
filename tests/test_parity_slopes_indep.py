"""Parity test: interlace vs R lme4, independent random intercept + slope.

Model:  y ~ x + (1 + x || g)
Data:   600 obs, 20 groups  (tests/fixtures/slopes_indep_data.csv)
Oracle: tests/fixtures/slopes_indep_r_results.json  (generated by gen_slopes_indep.R)
...
def data() -> pd.DataFrame:
    return pd.read_csv(FIXTURES / "slopes_indep_data.csv")


@pytest.fixture(scope="module")
def r_results() -> dict:
    return json.loads((FIXTURES / "slopes_indep_r_results.json").read_text())
...
def test_variance_components_diagonal(il_result, r_results):
    """Diagonal entries match within 5% rel_diff; off-diagonal should be zero."""
...
def test_variance_components_offdiagonal_zero(il_result, r_results):
    """Independent model: off-diagonal of covariance matrix is zero."""
    vc = il_result.variance_components["g"]
    np.testing.assert_allclose(vc.loc["(Intercept)", "x"], 0.0, atol=1e-12)
    np.testing.assert_allclose(vc.loc["x", "(Intercept)"], 0.0, atol=1e-12)
