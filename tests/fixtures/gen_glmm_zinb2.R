#!/usr/bin/env Rscript
# Generate zero-inflated NB2 GLMM reference data from glmmTMB.
#
# Model: y ~ x + (1 | group), family = nbinom2, ziformula = ~1
#
# The data is generated from a zero-inflated NB2 mixture:
#   - With probability pi, Y = 0  (structural zero)
#   - With probability (1-pi), Y ~ NB2(mu, theta)
#
# Usage:  Rscript tests/fixtures/gen_glmm_zinb2.R
...
beta0 <- 1.5        # intercept
beta1 <- 0.4        # slope
theta_true <- 2.0   # NB2 shape
pi_true <- 0.3      # zero-inflation probability

# Generate NB2 counts
eta <- beta0 + beta1 * x + u[group]
mu <- exp(eta)
lam <- rgamma(n, shape = theta_true, rate = theta_true / mu)
y_nb <- rpois(n, lambda = lam)

# Apply zero-inflation
zi <- rbinom(n, size = 1, prob = pi_true)
y <- ifelse(zi == 1, 0, y_nb)

df <- data.frame(y = y, x = x, group = factor(group))
write.csv(df, "tests/fixtures/zinb2_data.csv", row.names = FALSE)
...
results <- list(
  ...
  disp_params = as.list(fixef(fit)$disp),
...
write(toJSON(results, auto_unbox = TRUE, digits = 10, pretty = TRUE),
      "tests/fixtures/zinb2_results.json")

cat("Generated: zinb2_data.csv + zinb2_results.json\n")
...
cat(sprintf("  Theta (NB2 shape): %.4f\n", sigma(fit)))
